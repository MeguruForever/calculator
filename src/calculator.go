...
func Match(exp string)(string){
	...
	for {
		...
		//如果左右括号的个数相等
		if leftCount == rightCount{//这里有问题明天修改
			//如果左右括号的个数不为0
			if leftCount!=0{
				//截取括号中的表达式
				expression = exp[leftIndex+1:rightIndex]
				//计算括号中的表达式
				result = Match(expression)
				//将括号中的表达式替换为计算结果
				exp = exp[:leftIndex]+result+exp[rightIndex+1:]
				//重置index
				index = 0
				//重置左右括号的个数
				leftCount = 0
